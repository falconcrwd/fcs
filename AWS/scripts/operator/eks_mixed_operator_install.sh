...
# Falcon Operator EKS Mixed (Node + Fargate) Installation Script
...
#
# By default the script applies FalconDeploymentMixedAutoUpdate.yaml (which
# enables sensor auto-update). Set DEPLOYMENT_MANIFEST_NAME=FalconDeploymentMixed.yaml
# in eks_mixed_operator_inputs.txt to apply the non-auto-update manifest instead.
#
...
INPUTS_FILE="${SCRIPT_DIR}/eks_mixed_operator_inputs.txt"
...
# Default to the auto-update manifest if the inputs file did not set this
DEPLOYMENT_MANIFEST_NAME="${DEPLOYMENT_MANIFEST_NAME:-FalconDeploymentMixedAutoUpdate.yaml}"
DEPLOYMENT_MANIFEST="${SCRIPT_DIR}/${DEPLOYMENT_MANIFEST_NAME}"

if [[ ! -f "$DEPLOYMENT_MANIFEST" ]]; then
    log_error "Deployment manifest file not found: $DEPLOYMENT_MANIFEST"
    log_info "Set DEPLOYMENT_MANIFEST_NAME in $INPUTS_FILE to either"
    log_info "  FalconDeploymentMixedAutoUpdate.yaml  (auto-update enabled)"
    log_info "  FalconDeploymentMixed.yaml            (no auto-update)"
    exit 1
fi

log_info "Using deployment manifest: ${DEPLOYMENT_MANIFEST_NAME}"
...
# Get image tags from CrowdStrike registry. Mixed clusters need ALL FOUR images:
#   falcon-sensor    -> Node Sensor DaemonSet on EC2
#   falcon-container -> Container Sensor sidecar injected on Fargate
#   falcon-kac       -> Admission Controller on EC2
#   falcon-imageanalyzer -> Image Assessment at Runtime on EC2
...
log_success "Retrieved image tags - Sensor: $SENSOR_TAG, Container: $CONTAINER_TAG, KAC: $KAC_TAG, IAR: $IAR_TAG"

# Set registry paths
ECR_BASE="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${IMAGE_REPO_NAMESPACE}"

# Download and push images to ECR
log_info "Downloading and pushing Falcon Node Sensor image to ECR"
...
PROCESSED_MANIFEST="/tmp/${DEPLOYMENT_MANIFEST_NAME%.yaml}_processed.yaml"

# Copy original file to temporary location
cp "${DEPLOYMENT_MANIFEST}" "${PROCESSED_MANIFEST}"

# Use sed to substitute placeholders. The mixed manifest uses these placeholders:
...
# Replace the <TAG> placeholders per-image. Each image line appears on its own
# line, so we match using the image path to scope the replacement.
sed -i.bak \
