...
- **Sensor Update Policies** (Read) - required only if using `FalconDeploymentFargateAutoUpdate.yaml` (advanced `autoUpdate` enabled), so the Container Sensor sidecar can resolve the `linux-prod` update policy
