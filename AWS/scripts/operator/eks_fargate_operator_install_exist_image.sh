...
# Falcon Operator EKS Fargate Installation Script - Existing ECR Images
# This script installs the CrowdStrike Falcon Operator on an existing AWS EKS
# Fargate cluster and deploys FalconAdmission (KAC), FalconImageAnalyzer (IAR)
# and FalconContainerSensor (sidecar injector) using the
# FalconDeploymentFargate.yaml manifest.
#
# Use this version when the Falcon Container Sensor, KAC and Image Analyzer
# images are ALREADY present in your AWS Elastic Container Registry (ECR).
# The script skips downloading falcon-container-sensor-pull.sh, the
# CrowdStrike registry pull, and the ECR push steps. Image names and tags
# are read from eks_fargate_operator_inputs_exist_image.txt.
#
# Fargate-specific responsibilities still handled by this script:
...
#   - Substitutes both IAM role names AND image account/region/namespace/names/tags
#     into the FalconDeployment manifest before applying it. The Falcon Operator
#     then creates the service accounts with the eks.amazonaws.com/role-arn
#     annotations populated, enabling IRSA-based ECR pulls on Fargate (where
#     node-level credentials are unavailable).
...
INPUTS_FILE="${SCRIPT_DIR}/eks_fargate_operator_inputs_exist_image.txt"
...
# Use sed to substitute the AWS placeholders and IAM role names. The
...
# Replace the full image references for each of falcon-container, falcon-kac
# and falcon-imageanalyzer. We match each image line by the default image path
# component (falcon-container / falcon-kac / falcon-imageanalyzer) used in the
