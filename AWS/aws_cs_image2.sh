...
get_cs_image_list () {
  ...
  local cloud="${2}"                                                                                                                       # non-unified format
  local image_list="${image_list:-}"

  # ----- Begin non-unified format -----
  # Currently, there are inconsistencies in the image registry path across the different image types. So, some
  # special, custom logic is used to handle this problem. This makes the script inelegant :(

  # Image list URL based on image type
  local -A image_list_url
  image_list_url["falcon-sensor"]="https://${CS_REGISTRY}/v2/${IMAGE_TYPE}/release/${IMAGE_TYPE}/tags/list"                                # unified format
  image_list_url["falcon-container"]="https://${CS_REGISTRY}/v2/${IMAGE_TYPE}/release/${IMAGE_TYPE}/tags/list"                             # unified format
  image_list_url["falcon-kac"]="https://${CS_REGISTRY}/v2/${IMAGE_TYPE}/release/${IMAGE_TYPE}/tags/list"                                   # unified format
  image_list_url["falcon-imageanalyzer"]="https://${CS_REGISTRY}/v2/${IMAGE_TYPE}/${cs_cloud}/release/${IMAGE_TYPE}/tags/list"             # non-unified format

  disp_debug "${DEBUG_LEVEL_3}" "${FUNCNAME[0]}:$LINENO" "image_list_url=${image_list_url["${IMAGE_TYPE}"]}"

  image_list="$(curl \
  --silent \
  --request GET \
  --header "authorization: Bearer ${reg_bearer_token}" \
  --url "${image_list_url["${IMAGE_TYPE}"]}")"

  # ----- End non-unitified format-----

  # ----- Uncomment this block when everything is unified format -----
  # image_list="$(curl \
  # --silent \
  # --request GET \
  # --header "authorization: Bearer ${reg_bearer_token}" \
  # --url "${image_list_url["${IMAGE_TYPE}"]}")"
  # --url "https://${CS_REGISTRY}/v2/${IMAGE_TYPE}/release/${IMAGE_TYPE}/tags/list")"
...
get_cs_image_repo() {
  local cloud="${1}"                                                                                    # non-unified format
  ...
  # Image list URL based on image type
  local -A repo_url
  repo_url["falcon-sensor"]="${CS_REGISTRY}/${IMAGE_TYPE}/release/${IMAGE_TYPE}"                        # unified format
  repo_url["falcon-container"]="${CS_REGISTRY}/${IMAGE_TYPE}/release/${IMAGE_TYPE}".                    # unified format
  repo_url["falcon-kac"]="${CS_REGISTRY}/${IMAGE_TYPE}/release/${IMAGE_TYPE}"                           # unified format
  repo_url["falcon-imageanalyzer"]="${CS_REGISTRY}/${IMAGE_TYPE}/${cs_cloud}/release/${IMAGE_TYPE}"     # non-unified format

  echo "${repo_url["${IMAGE_TYPE}"]}"
  # ----- End non-unitified format-----

  # ----- Uncomment this line when everything is unified format -----
...
main() {
  ...
  cs_image_list="$(get_cs_image_list "${cs_reg_bearer_token}" "${cs_cloud}")"
